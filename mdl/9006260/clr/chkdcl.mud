...
<SETG LOWBOUND 1>
<MANIFEST DECL-RESTED DECL-ELEMENT DECL-ITEM-COUNT DECL-IN-REST
	  DECL-IN-COUNT-VEC DECL-REST-VEC HIGHBOUND LOWBOUND>

<SETG ALLWORDS '<PRIMTYPE FIX>>
...
<DEFINE VTS (X)
	<OR <AND <TYPE? .X ATOM>
		 ...
		 .X>
	    <AND <TYPE? .X ATOM> <GETPROP .X DECL>>
	    .X>>
			 ...
			 <COND (.TEM
				...
				<COND (.ORF <SET AL <PUT-IN .AL .TEM>>)
				      (ELSE
					   ...
					   <SET AL (.TEM !.AL)>>)>)>)
			(ELSE <RETURN T>)>)>)
	  (<NOT <EMPTY? <CHTYPE .TEM LIST>>> <RETURN .TEM>)>
...
<DEFINE FORMATCH (FRM RPAT "AUX" TEM (PAT .RPAT) EX) 
   ...
   <COND
    ...
    (ELSE
     <COND (<TYPE? .RPAT ATOM> <SET PAT <SET EX <GETPROP .RPAT DECL '.RPAT>>>)
	   (ELSE <SET RPAT <1 <CHTYPE .PAT LIST>>>)>
     <COND
      (<TYPE? .PAT ATOM>
       <SET TEM
...
<DEFINE MAP-MERGE ("TUPLE" PAIRS "AUX" (HIGH <2 .PAIRS>) (LOW <1 .PAIRS>)) 
	#DECL ((PAIRS) <TUPLE [REST FIX]> (HIGH LOW) FIX (VALUE) LIST)
	<REPEAT ()
...
<DEFINE RESTER? (S1 S2 FL FST SEGF
   ...
   <COND (<AND <OR .ORF <DECL-IN-COUNT-VEC .S2>>
	       <EMPTY? <CHTYPE <DECL-RESTED .S2> LIST>>
	       <NOT <DECL-IN-REST .S2>>>
	  <SET OPTIT T>)>
   <COND
...
<DEFINE GET-ELE-TYPE (DCL2 NN
   ...
   <PROG ()
     ...
     <AND <TYPE? .DCL1 ATOM> <SET DCL1 <GETPROP .DCL1 DECL '.DCL1>>>
...
<DEFINE MINL (DCL "AUX" (N 0) DD D DC (LN 0) (QOK <>) (ANDOK <>) TT (OROK <>)) 
   ...
   <AND <TYPE? .DCL ATOM> <SET DCL <GETPROP .DCL DECL '.DCL>>>
   <COND
   ...
   <PROG ()
	 ...
	 <AND <TYPE? .TYP ATOM>
	      <COND (<VALID-TYPE? .TYP> .TYP)
		    (<SET TYP <GETPROP .TYP DECL>> <AGAIN>)>>>>
	  ...
	  (<TYPE? .PAT ATOM>
	   <COND (<VALID-TYPE? .PAT>
		  <COND (<MEMQ <SET PAT <TYPEPRIM .PAT>>
			 ...
			 1)
			(ELSE 0)>)
		 (<SET PAT <GETPROP .PAT DECL>> <AGAIN>)
		 (ELSE 2)>)
